...
- [ ] Before first dev deploy: set wrangler secrets `TURNSTILE_SECRET_KEY`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY` (human-gated); decide dev-only code exposure (`SAC_E2E_EXPOSE_CODES`) vs wiring Resend first — without one of them, deployed auth codes are unreadable
