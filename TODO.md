...
- [ ] Human click-through of the live dev site pending: open the URL, pass the checkbox, verify with any test email (read the code at `/api/dev/last-code?email=<the email>`), complete an intake with an upload — proves live R2 presign + submit end-to-end in a real browser
