...
- [ ] BLOCKED ON USER (still): symlink creation remains EPERM after the Developer Mode attempt — re-check the toggle actually flipped (`ms-settings:developers`, switch says On, package finishes installing); `build:cf` + deploy dry-run wait on it
